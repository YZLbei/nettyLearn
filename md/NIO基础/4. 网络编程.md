...
* 但非阻塞模式下，即使没有连接建立，和可读数据，线程仍然在不断运行，白白浪费了 cpu**(缺点)**
...
* 绑定的事件类型可以有(四种事件类型)
  * connect - **客户端**连接成功时触发
  * accept - **服务器端**成功接受连接时触发（ServerSocketChannel只有一个）
...
- 事件注册之后会加入selector
- 在select之后会加入到SelectedKeys（selectedKeys就表示要处理的事件），在SelectedKeys中不会自动删除，所以**处理事件之后一定要手动删除**

所以要用迭代器，以便可以删除



#### 💡 cancel 的作用（处理客户端断开的情况）

> cancel 会取消注册在 selector 上的 channel，并从 keys 集合中删除 key 后续不会再监听事件

客户端断开需要，需要将key取消（从selector中真正删除key）
...
* 另一种思路是按分隔符拆分，缺点是效率低（因为需要一个字符一个字符对比）
...
关联扩容后的附件——attach（）（关联新的附件）（**会替换到原来的附件）**
...
* 每个 channel 都需要记录可能被切分的消息，因为 ByteBuffer 不能被多个 channel 共同使用，因此**需要为每个 channel 维护一个独立的 ByteBuffer**
...
- 先key.channel()打开ServerSocketChannel
...
也可以直接用ServerSocketChannel **accept（）**
...
处理accept事件之后，将新的通道（sc）注册，用于处理读写事件
...
* 创建 cpu 核心数的线程，每个线程配一个选择器，轮流处理 read 事件（worker）
...
1. 把sc.register(worker,SelectionKey.OP_READ);也放在worker线程中
2. 线程之间传递参数并让代码在某个位置执行，使用队列
3. worker初始化时要还行worker（selector），将sc注册进worker才可以接收读写事件
...
```java
workers[index.getAndIncrement()%workers.length()].regeister(sc);
```
